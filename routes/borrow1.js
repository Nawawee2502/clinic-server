...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({
            success: true,
            message: 'สร้างใบเบิกสินค้าสำเร็จ และเพิ่มข้อมูลใน STOCK_CARD แล้ว',
            data: {
                REFNO,
                TOTAL: total,
...
router.put('/:refno', async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            message: 'แก้ไขใบเบิกสินค้าสำเร็จ และเพิ่มข้อมูลใน STOCK_CARD แล้ว',
            data: {
                REFNO: refno,
                TOTAL: total,
                detailCount: details.length
...
router.delete('/:refno', async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            message: 'ลบใบเบิกสินค้าสำเร็จ และลบข้อมูลใน STOCK_CARD แล้ว'
        });
    } catch (error) {
        await connection.rollback();
